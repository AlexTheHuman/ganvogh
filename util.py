...
def brushstroke(d, where, angle, color, length, width):
    ...
    d.line(xy, fill=int(color, 16), width=width)
    return xy
...
def header(WELLS, WELL_CLEAR_HEIGHT):
    o = ""
...
def well_dip(c_index, WELLS, WELL_CLEAR_HEIGHT, DIP_HEIGHT, well_radius):
    o = ""
    o += "G0 Z%s; Go to well clear height on Z axis\n" % WELL_CLEAR_HEIGHT
...
def cluster(img, number_of_colors, size):
    np_array = util.image_resize(np.array(img), width=size)
...
def get_colors(img):
    ...
    for x in range(0,img.size[0]):
        for y in range(0,img.size[1]):
            g = util.c_to_string(pix[img.size[0]-1-x,img.size[1]-1-y])
            if g not in COLORS:
                COLORS.append(g)
    COLORS.sort(cmp=util.color_compare, reverse=True)
    return COLORS
...
def draw_palette(COLORS):
    ...
    for x, color in enumerate(COLORS):
        ...
        util.draw_circle(100 - (a + 75), (b + 25), int(color, 16), dr, 20)
    #img2.show()
...
def find_angle(original, canvas, color, where, brush_stroke_length, brush_stroke_width, min_angle=0, max_angle=180, step=12):
    ...
    for a in range(min_angle, max_angle, step):
        c = can.copy()
        d = ImageDraw.Draw(c)
        util.brushstroke(d, (r,r), a, color, brush_stroke_length, brush_stroke_width)
        del d
        error = mse(o, c)
        del c
